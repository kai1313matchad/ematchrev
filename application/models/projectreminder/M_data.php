<?php
	...
	class M_data extends CI_Model {
     ...
     ');
    	return $query;
	      ...
	      function tampil_data3($tgl1,$tgl2)
	      {
	      		...
	      		$query = $this->db->query('SELECT *, group_concat(distinct nama_dept SEPARATOR ", ") as deptini FROM reminder
     ...
     ');
	      ...
	      function tampil_data_reminder($dept_user)
	      {
	      	     //   $this->db->order_by("SCH_ID",'desc');
		           // return $this->db->get($table);
     ...
     ');
    	return $query;
	      ...
	      function tampil_data3_reminder($tgl1,$tgl2)
	      {
	      		...
	      		$query = $this->db->query('SELECT *, reminder.id as idr, group_concat(distinct nama_dept SEPARATOR ", ") as deptini,group_concat(distinct tgl_reminder SEPARATOR ", ") as tglreminder, jenis_reminder.nama_jenis as jenis FROM reminder
     join dept on find_in_set(dept.id_dept, reminder.dept)
     join jenis_reminder on jenis_reminder.id = reminder.jenis_reminder
     join tanggal_reminder on tanggal_reminder.id_reminder = reminder.id
     where reminder.tanggal_batas between "'.$tgl1.'" and "'.$tgl2.'"
     GROUP BY reminder.id
     order by reminder.id desc
     ');
