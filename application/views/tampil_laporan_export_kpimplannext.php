...
<body class="nilai semua">
	        ...
	        <div class="collapse navbar-collapse">
	            <ul class="nav navbar-nav">
                    ...
                    <li class="active dropdown">
	                    <a href="#" class="dropdown-toggle active" data-toggle="dropdown">Export KPIM Plann Next
	                    <span class="caret"></span>
	                    </a>
	                    <ul class="dropdown-menu"> 
	...
	<div class="container jarak">
		...
		<center><h1>Laporan KPIM Plan Next Semua Karyawan</h1></center>
		<br>
...
<!-- <div id='div1'>
	...
	<!-- </h4> -->
		...
		<div class="table-responsive">
			<table id="datakpimnext" style="background-color:lightgray;" class="display table table-striped table-bordered dt-responsive" cellspacing="0" width="100%">
		        <thead>
		            <tr>
		                <th style="text-align: center;">No.</th>
		                <th style="text-align: center;">Nama Karyawan</th>
		                <!-- <th style="text-align: center;">Nama Lengkap</th> -->
		                <th style="text-align: center;">Tgl. Plan Next</th>
		                <th style="text-align: center;">Goal</th>
		                <th style="text-align: center;">Description</th>
		                <th style="text-align: center;">Tgl. Deadline</th>
		                <th style="text-align: center;">Status Send</th>
	...
	<script>
        ...
        {
            $.ajax({
                url : "<?php echo site_url('Laporan/get_KPIM_Next_semua')?>",
                type: "POST",
                ...
                success: function(data)
                {
                    ...
                    {
                    	if (data[i]["tgl"] != tgl) {
                    		if (((data[i]["tgl"]).substr(0,4) != year) || ((data[i]["tgl"]).substr(5,2) != month)){
	                    		var trmont = $('<tr>').append(
	                    			$('<td class="text-left bg_h_bulan">').text(idBulanTahun((data[i]["tgl"]).substr(5,2) + "-" +(data[i]["tgl"]).substr(0,4))),
	                    			$('<td class="text-left bg_h_bulan">'),
	                    			$('<td class="text-left bg_h_bulan">'),
	                    			$('<td class="text-left bg_h_bulan">'),
	                    			$('<td class="text-left bg_h_bulan">'),
	                    			$('<td class="text-left bg_h_bulan">'),
	                    		...
	                    		year = (data[i]["tgl"]).substr(0,4);
	                    		month = (data[i]["tgl"]).substr(5,2);
	                    	}
                    		var trtgl = $('<tr>').append(
                    			$('<td class="text-left bg_h_hari">').text(namahari(data[i]["tgl"])+", "+tgl_indo(data[i]["tgl"])),
                    			$('<td class="text-left bg_h_hari">'),
                    			$('<td class="text-left bg_h_hari">'),
                    			$('<td class="text-left bg_h_hari">'),
                    			$('<td class="text-left bg_h_hari">'),
                    			$('<td class="text-left bg_h_hari">'),
                        ...
                        var tr = $('<tr>').append(
                        	...
                        	$('<td style="text-align: center;">').text(data[i]["id_status"])
                        	// $('<td>'+i+'</td>'),
        ...
        function idBulanTahun(mY)
        {
        	var bulantahun = '';
        	var bl = mY;
        	var marr = bl.split("-");
        	if (marr[0]=="01") {bulantahun = "Januari "+ marr[1]};
        	if (marr[0]=="02") {bulantahun = "Februari "+ marr[1]};
        	if (marr[0]=="03") {bulantahun = "Maret "+ marr[1]};
        	if (marr[0]=="04") {bulantahun = "April "+ marr[1]};
        	if (marr[0]=="05") {bulantahun = "Mei "+ marr[1]};
        	if (marr[0]=="06") {bulantahun = "Juni "+ marr[1]};
        	if (marr[0]=="07") {bulantahun = "Juli "+ marr[1]};
        	if (marr[0]=="08") {bulantahun = "Agustus "+ marr[1]};
        	if (marr[0]=="09") {bulantahun = "September "+ marr[1]};
        	if (marr[0]=="10") {bulantahun = "Oktober "+ marr[1]};
        	if (marr[0]=="11") {bulantahun = "November "+ marr[1]};
        	if (marr[0]=="12") {bulantahun = "Desember "+ marr[1]};
        	return bulantahun;
        }
