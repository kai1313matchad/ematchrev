...
<!-- 	                <?php 
                    ...
                    <table id="dataTablenext" class="table table-bordered table-hover table-striped" cellspacing="0" width="100%">
                        <thead class="text-center" style="background-color: #6db1ff">
                            <tr>
								...
								<th class="col-xs-3 text-center">Plan</th>
	...
	<!-- modal reminder -->
	<div class="modal" id="modal-reminder" tabindex="-1" role="dialog">
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <button type="button" class="close" data-dismiss="modal">&times;</button>
          	<h4 class="modal-title text-center" id="myModalLabel"><b>Tambah Reminder</b></h4>
	        </button>
	      </div>
	      <div class="modal-body">
	        <form id="form-reminder">
	        	<input type="hidden" name="idplan" id="idplan">
	        	<input type="hidden" name="idbobot_r" id="idbobot_r">
	        	<input type="hidden" name="tgl_r" id="tgl_r">
	        	<input type="hidden" name="deadline_r" id="deadline_r">
	        	<input type="hidden" name="desc_r" id="desc_r">
	        	<input type="hidden" name="dept_r" id="dept_r">
	        	<div class="form-group row">
	        		...
	        		<div class="col-lg-4">
	        			<div class='input-group date tgls' id='tgl_input'>     
							...
							<input type='text' id="tgl_kirim" class="form-control input-group-addon" name="tgl_kirim" placeholder="Tgl Kirim" >
						</div>
	        		</div>
	        	</div>
	        	<div class="form-group row">
	        		...
	        		<div class="col-lg-8">
	        			<select class="js-example-basic-multiple" id="myselect" style="width: 100%" name="email[]" multiple="multiple">
						</select>
	        		</div>
	        	</div>
	        	<div class="form-group row">
	      ...
	      <div class="modal-footer">
	        <button type="button" id="btn-add" class="btn btn-primary"><span class="glyphicon glyphicon-send"></span> Simpan</button>
	...
	<script>
		$(document).ready(function(){
			...
			//added by qoqom
			$('#tgl_start, #tgl_input').datetimepicker({
				useCurrent: false,
				format: 'YYYY-MM-DD',
				ignoreReadonly: true,
					...
					$.each(bulan,function(i,val){
						...
						$.each(hari,function(j,isi){
							...
							if($tb==$bulan){
								...
								$no = 1; var btn_reminder = '';
								for (var i = 0; i < data.length; i++){
									if(isi==data[i]["tgl"]){
										var tgl_in = Date.parse(data[i]["tgl"]);
										var tgl_dl = Date.parse(data[i]["deadline"]);
										<?php if ($this->session->userdata('level')!="2") { ?>
											btn_reminder = '<button type="button" onclick="get_reminder('+data[i]["id"]+')" class="btn btn-info btn-sm" class="btn btn-default" style="text-transform: capitalize;"> <span class="glyphicon glyphicon-send"></span> Reminder</button></td>';
										<?php } ?>
										
										var btn = (data[i]["id_approve"] != '1')?'<td class="text-center"><button type="button" onclick="edit_('+data[i]["id"]+')" class="btn btn-success btn-sm"><span class="glyphicon glyphicon-edit"></span> <text style="text-transform: capitalize;"> Edit</text></button> | <button type="button" onclick="hapus_('+data[i]["id"]+')" class="btn btn-danger btn-sm" class="btn btn-default" style="text-transform: capitalize;"> <span class="glyphicon glyphicon-trash"></span> Hapus</button>'+ btn_reminder :'<td class="text-center"><button type="button" class="btn btn-default btn-sm" disabled><span class="glyphicon glyphicon-edit"></span> <text style="text-transform: capitalize;"> Edit</text></button><button type="button" class="btn btn-default btn-sm" class="btn btn-default" style="text-transform: capitalize;" disabled> <span class="glyphicon glyphicon-trash"></span> Hapus</button></td>';
		...
		function dtables()
      	...
      	function get_reminder(id){
      		...
      		$.ajax({
      			url: "<?php echo site_url('C_sendemail/getData') ?>",
      			dataType: "JSON",
      			type: "POST",
      			data: {id: id},
      			success: function(data){
      				$('#idplan').val(data.id);
      				$("#idbobot_r").val(data.id_bobot);
      				$("#tgl_r").val(data.tgl);
      				$("#deadline_r").val(data.deadline);
      				$("#desc_r").val(data.action);
      				$("#dept_r").val(data.tgs_dept);
      				isi_email(data.tgs_dept);
      			}
      		});
      	}

      	function isi_email(id_dept){
			var myselect = $("#myselect");
			$('.js-example-basic-multiple').prop('disabled', false);
			$.ajax({
				url: "<?php echo site_url('C_sendemail/getEmail') ?>",
				type: "POST",
				dataType: "JSON",
				data: {id: id_dept},
				success: function(data){
		...
		// 	if (cek_validasi()) {
		// 		$.ajax({
		// 			url: "<?php echo site_url('C_sendemail/save') ?>",
		// 			type: "POST",
		// 			dataType: "JSON",
		...
		$('#btn-add').on('click', function(){
			if (cek_validasi()) {
				$.ajax({
					url: "<?php echo site_url('C_sendemail/send_email') ?>",
					type: "POST",
					dataType: "JSON",
					data: $("#form-reminder").serialize(),
					success: function(data){
		...
		function cek_validasi(){
			if ($("#tgl_kirim").val() == "") {
				alert("Tanggal pengiriman email harus diisi");
				$("#tgl_kirim").focus();
				return false;
			}
			if ($(".js-example-basic-multiple").val() == "") {
				alert("Email harus dipilih/diisi");
