...
class Reportsubnext2 extends CI_Controller {
 ...
 //        $this->load->view('tampil_pilihkaryawannext',$data);
    ...
    function get_allplannext()
    {
        ...
        $data = $this->db->join('dept b','b.id_dept = a.tgs_dept')->join('karyawan c','c.id_karyawan = a.id_karyawan')->where('a.id_karyawan = "'.$key.'" AND (tgl BETWEEN "'.$start.'" AND "'.$end.'")')->order_by('a.tgl','asc')->get('kpim_next a')->result();
