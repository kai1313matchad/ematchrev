...
class Factsheet extends CI_Controller {
	...
	public function __construct() {
		parent::__construct();
        $this->load->database();
        $this->load->model(array('M_kpimmingguan', 'M_factsheet', 'app_model'));
	...
	public function index()
	{
        ...
        $data['table'] = $this->M_factsheet->getPosting()->result();
		$this->load->view('t_factsheet', $data);
    ...
    public function laporan()
    {
        ...
        if (
            ($this->session->userdata('level') == 11 || $this->session->userdata('level') == 10) OR
            ($this->session->userdata('level') == 2 AND ($pos == false))
            )
        {
            $this->session->set_flashdata('message_name', 'Mohon maaf, Anda tidak dapat mengakses halaman laporan');
            redirect(base_url() . 'factsheet', 'refresh');
        }
        $data['semua'] = $this->M_factsheet->getsemuaPosting()->result();
        $this->load->view('t_factsheet_lap', $data);
    ...
    public function laporantgl()
    {
        $this->app_model->getLogin();
         if( $this->input->post('tglstart')==null  & $this->input->post('tglend')==null){
            redirect(base_url() . 'factsheet/laporan', 'refresh');
         }
        // $data['profilku'] = $this->M_kpimmingguan->getdataku()->result();
        ...
        $data['start'] = $tglstart;
        $data['end'] =$tglend;
        $data['semua'] = $this->M_factsheet->getPostingtgl($tglstart, $tglend)->result();
        $this->load->view('t_factsheet_lap', $data);
        // $this->load->view('t_recor', $data);
        // $this->load->view('upload_view');
    }
    ...
    public function detail($id)
    {
        ...
        $data['table'] = $this->M_factsheet->getdetailPosting($id)->result();
        $this->load->view('t_factsheet_detail', $data);
        // $this->load->view('t_recor', $data);
        // $this->load->view('upload_view');
    }
    ...
    public function terkirim()
    {
        $this->app_model->getLogin();
        // $data['profilku'] = $this->M_kpimmingguan->getdataku()->result();
        $data['semua'] = $this->M_factsheet->getPostingterkirim()->result();
        $this->load->view('t_factsheet_terkirim', $data);
        // $this->load->view('t_recor', $data);
        // $this->load->view('upload_view');
    }
    ...
    public function detailterkirim($id )
    {
        ...
        // $data['profilku'] = $this->M_kpimmingguan->getdataku()->result();
        $data['table'] = $this->M_factsheet->getdetailPosting($id)->result();
        $this->load->view('t_factsheet_detailterkirim', $data);
        // $this->load->view('t_recor', $data);
        // $this->load->view('upload_view');
    }
    ...
    public function laporantglterkirim()
    {
        ...
        $data['start'] = $tglstart;
        $data['end'] =$tglend;
        $data['semua'] = $this->M_factsheet->getPostingtglterkirim($tglstart, $tglend)->result();
        $this->load->view('t_factsheet_terkirim', $data);
        // $this->load->view('t_recor', $data);
        // $this->load->view('upload_view');
    }
    ...
    public function hapus($key){
        ...
        $this->M_factsheet->delete($key);

        // delete foto mulai
        $foto=$this->db->get_where('foto_factsheet',array('id_posting'=>$key));
        if ($foto->num_rows() > 0) {
            ...
            foreach ($n as $nm_foto) {
                if(file_exists($file=FCPATH.'/assets/upload-foto/factsheet/'.$nm_foto)){
                unlink($file);
                }    
            }
            
            $this->db->delete('foto_factsheet',array('id_posting'=>$key));
        }

        // mulai delete foto asli

        $foto_asli=$this->db->get_where('foto_factsheet_asli',array('id_posting'=>$key));
        if ($foto_asli->num_rows() > 0) {
            ...
            foreach ($n_asli as $nm_foto_asli) {
                if(file_exists($file_asli=FCPATH.'/assets/upload-foto/factsheet/'.$nm_foto_asli)){
                unlink($file_asli);
                }    
            }
            
            $this->db->delete('foto_factsheet_asli',array('id_posting'=>$key));
        }

        redirect(base_url() . 'factsheet', 'refresh');
    }

    public function hapuslaporan($key){
        $this->app_model->getLogin();
        // $where = array('id' => $key);
	...
	public function create() {
    	...
    	$data = array(
        	...
        	'jenis_factsheet' => $this->input->post('jenis_factsheet'),
        ...
        $this->M_factsheet->get_insert($data);
        redirect(base_url() . 'factsheet', 'refresh' );
    ...
    public function edit() {
        ...
        $this->M_factsheet->get_update($key, $data);
        redirect(base_url() . 'factsheet', 'refresh' );
    ...
    */
    ...
    function proses_upload($id_posting){
        ...
        $config['upload_path']   = './assets/upload-foto/factsheet/';
        ...
        if($this->upload->do_upload('userfile')){
            ...
            $this->db->insert('foto_factsheet',array('nama_foto'=>$nama, 'id_posting'=>$id_posting, 'id_kar_upload'=>$ygupload, 'jenis_foto'=>$jns_ft, 'token'=>$token));
    ...
    function proses_upload_asli($id_posting){
        ...
        if($this->upload->do_upload('userfile')){
            ...
            $this->db->insert('foto_factsheet_asli',array('nama_foto'=>$nama, 'id_posting'=>$id_posting, 'id_kar_upload'=>$ygupload, 'jenis_foto'=>$jns_ft, 'token'=>$token));
    ...
    public function lakukan_download($nama_foto){             
        force_download('./assets/upload-foto/factsheet/'.$nama_foto,NULL);
    }   

    public function lakukan_download_asli($token){
        $data = $this->db->get_where('foto_factsheet_asli', array('token' => $token))->result();
        foreach ($data as $key) {
            force_download('./assets/upload-foto/factsheet/'.$key->nama_foto,NULL);
        }
    ...
    function remove_foto(){
        ...
        $foto=$this->db->get_where('foto_factsheet',array('token'=>$token));


        if($foto->num_rows()>0){
            $hasil=$foto->row();
            $nama_foto=$hasil->nama_foto;
            if(file_exists($file=FCPATH.'/assets/upload-foto/factsheet/'.$nama_foto)){
                unlink($file);
            }
            $this->db->delete('foto_factsheet',array('token'=>$token));
    ...
    function remove_foto_asli(){
        ...
        $foto=$this->db->get_where('foto_factsheet_asli',array('token'=>$token));
        ...
        if($foto->num_rows()>0){
            ...
            $this->db->delete('foto_factsheet_asli',array('token'=>$token));
    ...
    function hapus_fotosatu($id){
        ...
        $foto=$this->db->get_where('foto_factsheet',array('id'=>$id));
        ...
        if($foto->num_rows()>0){
            ...
            $this->db->delete('foto_factsheet',array('id'=>$id));
    ...
    function hapus_fotosatu_asli($tokenku){
        ...
        $foto=$this->db->get_where('foto_factsheet_asli',array('token'=>$tokenku));
        ...
        if($foto->num_rows()>0){
            ...
            $this->db->delete('foto_factsheet_asli',array('token'=>$tokenku));
    ...
    public function updatestatus(){
        ...
        $this->M_factsheet->updatestatus($key, $id, $data);
