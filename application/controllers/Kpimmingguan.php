...
class Kpimmingguan extends CI_Controller {
	...
	public function __construct() {
		...
		parent::__construct();
        $this->load->model(array('M_kpimmingguan', 'M_pengumuman', 'M_kpimmingguannext', 'M_karyawanku', 'M_reportsub','M_sendemail','app_model'));
        $this->load->library(array('session', 'form_validation', 'upload', 'user_agent', 'email'));
        $this->load->helper(array('url', 'form', 'text', 'html', 'security', 'file', 'directory', 'number', 'date', 'download', 'tgl_indo','2019'));
	}
	...
	public function index()
	{
        ...
        //cek auto send reminder
        $this->M_sendemail->sendAutomatically();
        //end cek auto send
    ...
    public function ijin_sdm()
    {
       ...
       //mulai dept
        ...
        $this->load->view('tampil_ijin_publik',$data);
    ...
    function add_kpim()
    {
        ...
        $num=str_replace(',','',$this->input->post('omset'));
        $omset=floatval($num);
        ...
        $ins = array(
            ...
            'lokasi_mkt'=>$this->input->post('lokasi'),
            'omset_mkt'=>$omset,
            'klien_mkt'=>$this->input->post('klien'),
...
// $tgl = $this->input->post('tgl');\
    ...
    //edited by qoqom
    $id_bobot = $this->input->post('goals');
    ...
    // $bobot = $gl2[1];
    //edited by qoqom
    $q = $this->db->query("SELECT * FROM master_bobot WHERE id_bobot='$id_bobot'")->result_array();
    $goals = $q[0]['nama'];
    $bobot = $q[0]['bobot'];
        ...
        $data1=$this->M_kpimmingguan->get_insert($data);
        
        //upadte sistem - kolom khusus Marketing
	...
	public function update($key){
        ...
        // $bobot = $gl2[1];
        //added by qoqom
        $id_bobot = $this->input->post('goaledit');
        $q = $this->db->query("SELECT * FROM master_bobot WHERE id_bobot='$id_bobot'")->result_array();
        $goals = $q[0]['nama'];
        $bobot = $q[0]['bobot'];

        $num=str_replace(',','',$this->input->post('omsetedit'));
        $omset=floatval($num);
		...
		$data = array(
            ...
            //Update Sistem Halaman Input KPIM Mingguan (Marketing)
            'lokasi_mkt'=>$this->input->post('lokasiedit'),
            'omset_mkt'=>$omset,
            'klien_mkt'=>$this->input->post('klienedit'),
    ...
    public function tampil_ijin_KPIM(){
        $tglstart = $this->input->post('tglstart');
        $tglend = $this->input->post('tglend');
        $this->db->join('karyawan b','b.id_karyawan = a.id_karyawan');
        $this->db->join('dept c','c.id_dept = a.tgs_dept');
        $this->db->from('kpim_karyawan a'); 
        $this->db->where_in('nama_goals',array('Ijin','Sakit','Dinas'));
        if (($tglstart != null) and ($tglend != null)){
            $this->db->where('a.tgl >=',$tglstart);
            $this->db->where('a.tgl <=',$tglend);
        }
        $this->db->where('action !=','Lain-Lain');
        $this->db->order_by('a.tgl','desc');
        $this->db->order_by('a.id_karyawan','asc');
        // $this->db->order_by('a.tgs_dept','asc');
    ...
    public function getKpimTerkirim()
    {
        ...
        $this->db->where("tgl BETWEEN '".$startdate."' and '".$enddate."'");
        $this->db->order_by("tgl desc","id asc");
        $query = $this->db->get();
        $data   = $query->result();
        echo json_encode($data);
