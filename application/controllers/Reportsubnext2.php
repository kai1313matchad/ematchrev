...
class Reportsubnext2 extends CI_Controller {
 ...
 //        $this->load->view('tampil_pilihkaryawannext',$data);
    ...
    function get_allplannext()
    {
        ...
        $data = $this->db->join('dept b','b.id_dept = a.tgs_dept')->join('karyawan c','c.id_karyawan = a.id_karyawan')->join('master_bobot d','d.id_bobot = a.id_bobot','left')->where('a.id_karyawan = "'.$key.'" AND (tgl BETWEEN "'.$start.'" AND "'.$end.'")')->order_by('a.tgl','asc')->get('kpim_next a')->result();
        echo json_encode($data);
    }

    function get_allplannext_send()
    {
        $this->app_model->getLogin();
        $key = $this->input->post('subordinat');
        $start = $this->input->post('tglstart');
    ...
    function disapprove_plannext($id)
    {
        $this->app_model->getLogin();
        $upplan = array(
            'id_status'=>'1',
