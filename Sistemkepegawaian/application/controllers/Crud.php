<?php
	...
	class Crud extends CI_Controller 
	{
		...
		{			
			$que = $this->db->get_where('user_group_hc','usg_dtsts = "1"');
			$data = $que->result();
			echo json_encode($data);
		}
		...
		{
			$this->db->from('user_hc');
			$this->db->join('user_group_hc','user_group_hc.usg_id = user_hc.usg_id');
			$this->db->where('user_hc.kar_id',$id);
			$que = $this->db->get();
			$data = $que->result();
			echo json_encode($data);
		}
		...
		{			
			$que = $this->db->select('id_cab,nama_cabang');
			$que = $this->db->from('cabang');
			$que = $this->db->get();
			$data = $que->result();
			echo json_encode($data);
		}
		...
		{
			$data = $this->crud->get_by_id2('user_hc','karyawan ',array('user_hc.usr_id'=>$id),'karyawan.id_karyawan = user_hc.kar_id');
			echo json_encode($data);
		}
		...
		{
			$data = $this->crud->get_by_id2('user_hc','user_group_hc',array('usr_id'=>$id),'user_hc.usg_id = user_group_hc.usg_id');
			echo json_encode($data);
		}
