<?php
	...
	class Searchdata extends CI_Controller
	{
		public function __construct()
		{
			...
			$this->load->model('datatables/search/Dt_srchkrybyid','s_krybyid');
		...
		public function srch_kryby_id_kry()
		{
			$id = $this->input->post('sts');
			$id_kry = $this->input->post('id_kry');
			$br = $this->input->post('brch');
			$chk = $this->input->post('chk');
			$brc = 'a.id_branch = '.$br;
			$list = $this->s_krybyid->get_datatables($id_kry,$brc);
			$data = array();
			$no = $_POST['start'];
			switch ($chk) {
				case '0':
			...
			$output = array(
							"draw" => $_POST['draw'],
							"recordsTotal" => $this->s_krybyid->count_all(),
							"recordsFiltered" => $this->s_krybyid->count_filtered($id_kry,$brc),
							"data" => $data,
					);			
			echo json_encode($output);
		}
		...
		public function srch_ijin()
		{
			...
			$brc = 'a.branch_id = '.$br;
			if ($chk==='0') {
				$list = $this->s_ijinbysts->get_ijin_datatables($id,$brc);
			}else{
