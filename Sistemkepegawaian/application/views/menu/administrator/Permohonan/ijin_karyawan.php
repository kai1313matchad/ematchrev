...
<link rel="stylesheet" href="//cdnjs.cloudflare.com/ajax/libs/timepicker/1.3.5/jquery.timepicker.min.css">
                    ...
                    <div class="col-sm-12 col-xs-12">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-offset-0 col-sm-2 text-center">
                                <a href= "javascript:void(0)"><button id="btnAprhc" onclick="appr_ijin()" class="btn btn-block btn-primary btn-default ">Approve HC</button></a>
                            </div>
                            <div class="col-sm-offset-1 col-sm-2 text-center">
                                <a href="javascript:void(0)"><button id="btnDishc" onclick="disappr_ijin()" class="btn btn-block btn-danger btn-default ">Disapprove</button></a>
                            </div>
                            <?php } ?>
                            <div class="col-sm-offset-1 col-sm-2 text-center">
    ...
    <script>
        ...
        {   
            $('#btnAdd').show();
            $('#btnApr,#btnUpd,#btnDis,#btnCancel,#btnAprhc,#btnDishc').hide();
            $('#dtp1').datetimepicker({                
                format: 'YYYY-MM-DD'
            });
            $('#dtp2').datetimepicker({                
        ...
        function pick_ijinedit(id)
        {
            ...
            $.ajax({
                ...
                success: function(data)
                {   
                    $("#atasan").empty(); $("#atasan").prop('disabled', false);
                    dropatasan();
        ...
        function pick_ijinapr(id)
        {
            $.ajax({
                ...
                {      
                    ...
                    $('#modal_srch_ijin').modal('hide');
                    $('#btnAdd').hide();
                    $('#btnApr, #btnDis').css({'display':'block'});
                    // $('#btnUpd').hide();
                    // $('#btnCancel').hide();
                    ...
                    if (data.acc_atasan == "Tidak") {
                        $("#btnAprhc,#btnApr, #btnDis,#btnDishc").hide();
                    }else if(data.acc_atasan == "Ya" && data.acc_hc == ""){
                        $("#btnAprhc, #btnDishc").show();
                        $("#btnApr, #btnDis").hide();
                    }else if(data.acc_atasan == "Ya" && data.acc_hc != null){
                        $("#btnAprhc, #btnDishc").hide();
                        $("#btnApr, #btnDis").hide();
                    }
                    if (data.acc_hc == "Ya") {
                        $('#btnApr, #btnDis').css({'display':'none'});
                    }
                    ambilatasan(data.id_atasan);
                    // alert(data.id_atasan);
                    $("#atasan option[value='"+data.id_atasan+"']").attr('selected','selected');
                    $("#atasan").prop('disabled','disabled');
                    $('[name="branch"]').val(data.id_branch);
                    $('[name="dept"]').val(data.dept);
                    $('[name="jabatan"]').val(data.jabatannya);
